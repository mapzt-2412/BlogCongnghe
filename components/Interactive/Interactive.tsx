...
const Interactive: FC<IInteractiveProps> = ({ dataInteractive, id, type }) => {
  ...
  useEffect(() => {
    ...
    dataInteractive?.isBookmark,
    dataInteractive?.likeNum,
    dataInteractive?.dislikeNum,
    dataInteractive?.liked,
    dataInteractive?.disliked,
    dataInteractive?.likeNum,
    dataInteractive?.dislikeNum,
    dataInteractive,
  ]);
  ...
  useEffect(() => {
    if (callApi) {
      PropertiesService.likeArticle(data, getToken()).then((data) =>
  ...
  const handleInteractive = (type) => {
    if (type === "like") {
      setLike(!like);
      userService
        .toggleLike({ articleId: id, status: !like }, getToken())
        .then((data) => {
          console.log(data);
      ...
      setDislike(!dislike);
      userService
        .toggleDisliked({ articleId: id, status: !dislike }, getToken())
        .then((data) => {
          setLikeNum(data.data.data.likeNum);
          setDislikeNum(data.data.data.dislikeNum);
          setLike(data.data.data.like);
  ...
  const handleReport = () => {
    PropertiesService.sendReport(
      { articleId: id, description: reportContent, type: placement },
      getToken()
    ).then((data) => {
  ...
  return (
    <div className="interactive-container">
      <button
        ...
        onClick={() => handleInteractive("like")}
      >
        {like ? (
          <IconLikeActive width={23} height={21} />
        ) : (
      ...
      <button
        className={"interactive dislike"}
        onClick={() => handleInteractive("dislike")}
      >
        {dislike ? (
          <IconLikeActive width={23} height={21} />
        ) : (
      ...
      {!type && (
        <>
          <FacebookShareButton
            url={`${process.env.REACT_APP_BASE_URL}/post/${id}`}
            className="interactive"
          >
          ...
          <button
            className="interactive"
            onClick={() => handleInteractive("bookmark")}
          >
      ...
      >
        ...
        <p>
          ...
          ngay sự giúp đỡ trước khi báo cáo với chúng tôi.
        </p>
        <Radio.Group value={placement} onChange={placementChange}>
          {report?.map((value, index) => (
            <Radio.Button value={value.type} key={index}>
              {value.value}
            </Radio.Button>
          ))}
        </Radio.Group>
