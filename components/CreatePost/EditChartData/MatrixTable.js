...
const MatrixTable = ({col, row, setRow, setCol, setData, data}) => {
    ...
    const renderCol = (length, index) => {
        ...
        for (let i = 0; i < length; i++) {
          data.push(
            <td>
              <Input name={`input${i+2}`} onChange={handleChangeInput} id={index+1}
                value={
                  dataTable[`input${i+2}`] ? dataTable[`input${i + 2}`][index + 1] : ""
                }
              />
            </td>
          );
    ...
    const renderDataset = (length) => {
        ...
        for (let i = 0; i < length; i++) {
          data.push(
            <td>
              <Input name={`dataset${i+2}`} onChange={handleChangeDataset} value={dataset ? dataset[`dataset${i + 2}`] : ""}/>
            </td>
          );
        }
        return data;
    ...
    const renderRow = (length) => {
        ...
        for (let i = 0; i < length; i++) {
          data.push(
            <tr>
                <td>
                    <Input name={`lable${i + 2}`} onChange={handleChangeLabel} value={lable ? lable[`lable${i + 2}`] : ""}/>
                </td>
                <td>
                    <Input name={`input${1}`} onChange={handleChangeInput} id={i+2} 
                    value={
            ...
            <div className="maxtrix-content">
                <table>
                    <thead>
                        ...
                        <td>
                            <Input name={`dataset${1}`} onChange={handleChangeDataset}
                    ...
                    <tbody>
                        <tr>
                        <td>
                            <Input name={`lable${1}`} onChange={handleChangeLabel} 
