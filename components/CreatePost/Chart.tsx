...
import { Modal, Row, Col, Radio, Button, RadioChangeEvent, InputNumber } from "antd";
...
const Chart : FC<IChartProps> = ({
  ...
  const handleOk = () => {
    if(addData){
      addData({
        lable: <ChartWrapper type={typeChart} dataTable={data} isTable={false}/>,
        title: "Biểu đồ",
      });
      setTypeChart("");
      setIsType(false);
    ...
    <div className="chart-wrapper">
      ...
      >
        <div className="chart-container">
          <div className="chart-title">Bảng biểu</div>
              ...
              <>
                <div className="modal-chart-input">
                  <div className="modal-chart-input-col">
                    <p>Chọn số cột:</p>
                    <InputNumber min={1} controls={false} value={col} onChange={(value) => setCol(value)}/>
                  </div>
                  <div className="modal-chart-input-row">
                    <p>Chọn số hàng:</p>
                    <InputNumber min={1} controls={false} value={row} onChange={(value) => setRow(value)}/>
                  </div>
                </div>
                <div className="modal-chart-type">
                  ...
                  <Row gutter={16}>
      <Col span={4}>
        <div className={`type-item ${typeChart==="bar-chart" && "active"}`} onClick={() => onChangeType("bar-chart")}>
          <IconBarChart/>
        </div>
      </Col>
      <Col span={4}>
        <div className={`type-item ${typeChart==="bar-chart-horizontal" && "active"}`}onClick={() => onChangeType("bar-chart-horizontal")}>
        <IconBarChartHorizontal/>
        </div>
      </Col>
      <Col span={4}>
      <div className={`type-item ${typeChart==="bubble-chart" && "active"}`} onClick={() => onChangeType("bubble-chart")}>
        <IconBubbleChart/>
        </div>
      </Col>
      <Col span={4}>
      <div className={`type-item ${typeChart==="area-chart" && "active"}`} onClick={() => onChangeType("area-chart")}>
        <IconAreaChart/>
        </div>
      </Col>
      <Col span={4}>
      <div className={`type-item ${typeChart==="doughnut-chart" && "active"}`} onClick={() => onChangeType("doughnut-chart")}>
        <IconDoughnutChart/>
        </div>
      </Col>
      <Col span={4}>
      <div className={`type-item ${typeChart==="line-chart" && "active"}`} onClick={() => onChangeType("line-chart")}> 
        <IconLineChart/>
        </div>
      </Col>
    </Row>
            ...
            {page === 2 && (
              ...
              //   isEdit={isEdit}
              // />
              <MatrixTable col={col} row={row} setCol={setCol} setRow={setRow} setData={setData} data={data}/>
            )}
            {
              page === 3 && (
        ...
        <div className="modal-chart-button-wrapper">
                {
                  page !==1 &&
                  <div className={`modal-chart-button ${!isType && "disable"}`} onClick={backPage}>
                    <IconBack width={24}/>
                    <p>Trở lại</p>
                  </div>
                }
                <div className={`modal-chart-button next ${!isType && "disable"}`} onClick={changePage}>
                    <IconBack width={24}/>
                    <p>Tiếp tục</p>
                </div>
                </div>
